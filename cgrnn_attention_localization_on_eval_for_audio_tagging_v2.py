...
#att = TimeDistributed(Dense(128,activation='relu'))(flat)
att = TimeDistributed(Dense(1,activation='sigmoid'))(input_flat)
att = Flatten()(att)
#att=Lambda(lambda x: K.dot(x, 0.5),output_shape=(249,))([att])
att1 = RepeatVector(128)(att)
att1=Permute((2,1))(att1)
att2 = RepeatVector(8)(att)
att2=Permute((2,1))(att2)

flat_att = merge([flat, att1], mode='mul') #Merge is for model, while merge is for tensor.  # concat_axis=2 # two matrix have same dims, 'mul' should be dot2dot mult
...
det =TimeDistributed(Dense(8,activation='softmax'))(flat_att) # The posterior sum of each tag is 1.0, now the dims of det are 33 frs * 8 tags

lstm1=Bidirectional(GRU(output_dim=128,return_sequences=True,dropout_W=0.0, dropout_U=0.0))(flat_att)
#print model.output_shape
lstm2=Bidirectional(GRU(output_dim=128,return_sequences=True,dropout_W=0.0, dropout_U=0.0))(lstm1)
lstm3=Bidirectional(GRU(output_dim=128,return_sequences=True,dropout_W=0.0, dropout_U=0.0))(lstm2)


#merge1=Flatten()(merge1)
dnn=TimeDistributed(Dense(500,activation='relu'))(lstm3)
dnn=TimeDistributed(Dropout(0.2))(dnn)
dnn=TimeDistributed(Dense(n_out,activation='sigmoid'))(dnn)
dnnout = merge([dnn, det], mode='mul') 
dnnout=merge([dnnout, att2], mode='mul')
...
dump_fd=cfg.scrap_fd+'/Md/AttentionFlatAtt_v2_detection_clip_utt_keras_overlap50_eva816_1CNN128onMBK40_noILD_weights.{epoch:02d}-{val_loss:.2f}.hdf5'
...
allmodel.fit(tr_X1, tr_y, batch_size=100, nb_epoch=31,
